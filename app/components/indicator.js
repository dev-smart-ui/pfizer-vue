...
app.component('indicator', {
    ...
    methods: {
        startPos() {
            ...
            let angelFullTriangel = 2 * Math.asin(widthCss / (2 * R)) * 180 / Math.PI
            let okrL = Math.PI * R * angelFullTriangel / 180 // lendth okruznost
            let yTriangel = (R - verticalHordaFN(R, widthCss, widthCss)) - (R - verticalHordaFN(R, widthCss, curentLendth) )
            let curentHorda = Math.sqrt((curentLendth*curentLendth) + (yTriangel * yTriangel))
            let radCurent = 2 * Math.asin(curentHorda / (2 * R)) * 180 / Math.PI
            let curOkrL = Math.PI * R * radCurent / 180 // lendth okruznost
            this.item.controlBaseData = Math.round( this.indicatorGrid - (curOkrL * this.indicatorGrid / okrL   ))
            /************/
        ...
        editorGridSize() {
            ...
            } else if(this.currentGridSize == 'day' || this.currentGridSize == 'year'){
                if(this.item.controlBaseData == '1'){
                    return ' ' + this.currentGridSize
                } else {
                    return ' ' + this.currentGridSize + 's'
                }
            }
        },
    },
